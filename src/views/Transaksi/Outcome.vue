  ...
  <AdminLayout>
    ...
    <div class="px-6 py-8 dark:bg-gray-900 min-h-screen">
      ...
      <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mb-6">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">Tambah Pengeluaran Baru</h2>
        <form @submit.prevent="addTransaction">
          ...
          <div class="flex justify-end">
            <button type="submit" :disabled="bookkeepingStore.loading" class="px-5 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed">
              {{ bookkeepingStore.loading ? 'Menyimpan...' : 'Tambah Pengeluaran' }}
            </button>
          </div>
        </form>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-if="bookkeepingStore.expenseTransactions.length === 0 && !bookkeepingStore.loading">
...
const pageTitle = ref('Pengeluaran');
const breadcrumbItems = ref([
  { label: 'Pembukuan', path: '/expense' },
  { label: 'Pengeluaran', path: '/expense' }
]);
...
const addTransaction = async () => {
  ...
  const transactionData = {
    ...
    type: 'pengeluaran' as const, // <<< PERBAIKAN: Menggunakan 'pengeluaran'
