<template>
  <AdminLayout>
    ...
    <div class="px-6 py-8 dark:bg-gray-900 min-h-screen">
      <h1 class="text-3xl font-bold mb-6 text-gray-800 dark:text-white">Laporan Laba Rugi (Sederhana)</h1>
      <p class="text-gray-600 dark:text-gray-400 mb-6">
        Ringkasan total pemasukan dan pengeluaran dalam periode waktu tertentu.
      </p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
          <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Total Pendapatan</h3>
          <p class="text-4xl font-extrabold text-green-600 dark:text-green-400"> {{ formatRupiah(bookkeepingStore.totalIncome) }}</p>
          <p class="text-gray-500 dark:text-gray-400">Jumlah pemasukan dalam periode ini</p>
        </div>
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
          <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Total Beban</h3>
          <p class="text-4xl font-extrabold text-red-600 dark:text-red-400"> {{ formatRupiah(bookkeepingStore.totalExpense) }}</p>
          <p class="text-gray-500 dark:text-gray-400">Jumlah pengeluaran dalam periode ini</p>
        </div>
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
          <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Laba Bersih / Rugi Bersih</h3>
          <p :class="['text-4xl font-extrabold', { 'text-green-600 dark:text-green-400': netProfit >= 0, 'text-red-600 dark:text-red-400': netProfit < 0 }]">
             {{ formatRupiah(netProfit) }}
          </p>
          <p class="text-gray-500 dark:text-gray-400">Performa keuangan Anda</p>
        </div>
      </div>
    </div>
  </AdminLayout>
...
const pageTitle = ref('Laba Rugi');
const breadcrumbItems = ref([
  { label: 'Laporan Keuangan', path: '/laporan/laba-rugi' }, // Sesuaikan path jika ada halaman utama laporan
  { label: 'Laba Rugi', path: '/laporan/laba-rugi' }
]);
